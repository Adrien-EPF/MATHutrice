...
On the first start the empty database is seeded with the notions and skills, and with one user per role (`student@epfedu.fr`, `teacher@epf.fr`, `admin@epf.fr`), so the home page lists the modules.
